...
set.seed(10)
tpars <- read.csv("pars_tweedie_viz.csv") |>
...
tweedie_viz <- (tpars_plots[[1]] + tpars_plots[[2]]) /
...
ggsave("output/tweedie_viz.png", tweedie_viz, width = 11.1, height = 7.2)
...
mod_tw <- brms::brm(
  vals ~ 1, data = test_dat, family = tweedie, stanvars = stanvars,
  backend = "cmdstanr"
)
mod_hg <- brms::brm(
